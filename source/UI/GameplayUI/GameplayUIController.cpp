...
namespace UI
{
    namespace GameplayUI
    {
        ...
        void GameplayUIController::initializeText()
        {
            ...
            // Convertir font_size de float a int de forma explícita
            int font_size_int = static_cast<int>(font_size);

            score_text->initialize(score_string, sf::Vector2f(score_text_x_position, text_y_position), FontType::BUBBLE_BOBBLE, font_size_int, text_color);
            enemies_killed_text->initialize(enemies_killed_string, sf::Vector2f(enemies_killed_text_x_position, text_y_position), FontType::BUBBLE_BOBBLE, font_size_int, text_color);
        }
